...
import {useForm} from "courses/TestingRoutesHooks/src/hooks/useForm/useForm";
...
test("values should be updated when field changes", () => {
    ...
    act(() => field.onChange({target: {value: "test"}}))
...
test("submit callback should be called when form submitted", () => {
    ...
    act(() => field.onChange({target: {value: "test"}}))
    act(() => result.current.handleSubmit(onSubmit)({preventDefault: jest.fn()}))
